...
' 18.02.28 - File item prefix
'          - Please note, this program is now only maintained in BlitMax because no good alternative has yet arrived.
'            A good Go candidate suffers from security conflicts, Lazarus doesn't compile well and the community is
'            not helping me on that, but rather rude about it, so I am basically looking for a good simple to set up
'            alternative for GUI based programs. If worst comes to worst, I may need to set something up in the LOVE engine
'            but as that engine is not set up for this kind of programming, I hope not to have to go down that road.
...
For RK = EachIn MapKeys(Recs)
	...
	WriteLine bt,"~tlocal ret = {"; FF=False
...
Update
Print "Lua output: "+OutputLuaBase
